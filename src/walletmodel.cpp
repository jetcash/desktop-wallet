...
void WalletModel::statusReceived(const RpcApi::Status& status)
{
    ...
    if (status.recommended_fee_per_coin != pimpl_->status.recommended_fee_per_coin)
        changedRoles << ROLE_RECOMMENDED_FEE_PER_COIN;
...
//        return pimpl_->status.lower_level_error;
        return pimpl_->status.lower_level_error.isEmpty() ? tr("Connected to jetcashd") : tr("Jetcashd status: %1").arg(pimpl_->status.lower_level_error);
    ...
    case COLUMN_RECOMMENDED_FEE_PER_COIN:
        return tr("Recommended fee per coin: %1").arg(pimpl_->status.recommended_fee_per_coin);
...
QVariant WalletModel::getUserRoleStatus(const QModelIndex& /*index*/, int role) const
{
    ...
    case ROLE_RECOMMENDED_FEE_PER_COIN:
        return pimpl_->status.recommended_fee_per_coin;

...
QString BuiltinWalletd::errorMessage(ReturnCode err)
{
    ...
    switch(err)
    {
    case ReturnCode::JETCASHD_DATABASE_ERROR:
        msg = tr("Database write error. Disk is full or database is corrupted.");
        break;
    case ReturnCode::JETCASHD_ALREADY_RUNNING:
        msg = tr("Cannot run jetcashd. Another instance of jetcashd is running.");
        break;
    case ReturnCode::WALLETD_BIND_PORT_IN_USE:
        msg = tr("Cannot run walletd. Walletd bind port in use.");
        break;
    case ReturnCode::JETCASHD_BIND_PORT_IN_USE:
        msg = tr("Cannot run jetcashd. Jetcashd bind port in use.");
        break;
    ...
    case ReturnCode::WALLET_FILE_EXISTS:
        msg = tr("The specified wallet file already exists. Jetcash wallet could not overwrite an existed file for safety reason. If you want to overwrite the file please remove it manually and try again.");
        break;

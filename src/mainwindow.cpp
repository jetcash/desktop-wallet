...
//const char COMMUNITY_FORUM_URL[] = "https://jetcashtalk.org";
const char COMMUNITY_FORUM_URL[] = "https://jetcash.org";
const char REPORT_ISSUE_URL[] = "https://jetcash.org/contact";

const char BUTTON_STYLE_SHEET[] =
        "QPushButton {border: none;}"
        "QPushButton:checked {background-color: #F5A623; color: #FFFFFF}";
}
    ...
    , walletModel_(walletModel)
{
    ...
    setWindowIcon(QIcon(":images/jetcash_lin"));
    clearTitle();
    ...
    // Scale balance icon 
    QPixmap balanceLogo = QPixmap(QString(":icons/light/balance"));
    balanceLogo.scaledToWidth(36);
    m_ui->m_balanceIconLabel->setPixmap(balanceLogo);
...
//    qDebug("raw tx = %s", qPrintable(tx.binary_transaction));
    ...
    for (const RpcApi::Transfer& tf: tx.transaction.transfers)
    {
        ...
        msg.append(tr("%1 JTC to %2\n").arg(amountStr).arg(addressStr));
    }
    msg.append(tr("Fee: %1 JTC\n").arg(formatAmount(tx.transaction.fee)));
    msg.append(tr("Total send: %1 JTC").arg(formatAmount(-ourAmount)));

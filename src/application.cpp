...
{
    setApplicationName("jetcash"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Jetcash Wallet"));
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("jetwallet.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Jetcash wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Jetcash wallet GUI already running"));
        return false;
    }
...
void WalletApplication::initSystemTrayIcon()
{
    ...
    m_systemTrayIcon->setIcon(QIcon(":images/jetcash_lin"));
...
{
    splashMsg(tr("Looks like this is your first run of the new Jetcash Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote walletd daemon."));
}
